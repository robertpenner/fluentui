...
See the [SpinBlur example](?path=/docs/motion-components-preview-atoms-spinblur--docs) for a custom presence component built from `rotateAtom` + `blurAtom` + `scaleAtom`.
